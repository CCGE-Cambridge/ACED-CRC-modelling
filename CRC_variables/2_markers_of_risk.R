...
get_df_gp_marker_events <- function(lma) {
  ...
  df_gp_marker_events <- df_gp_marker_events %>% select(eid, ends_with("in_lb"), ends_with("new_onset"))
...
##temp <- load(file = file.path(res_dir, paste0("marker_events/df_gp_marker_events_lma60.Rdata")))
