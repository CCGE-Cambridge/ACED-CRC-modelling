...
source("~/ACED-RREDD-EHR/sam/scripts/0_config_nomlr3.r")
...
#source("~/ACED-RREDD-EHR/CRC_variables/dependencies/multimorb_score_info_and_funcs.r")
...
get_df_gp_multimorb <- function(lma) {
  ...
  df_script_all <- df_script_all %>% 
   ...
   ##calculate mm age categories (note I don't include the under 40 age categories)
    ...
    df_mm_hosp_long <- mm_score_HR(df_mm_covars, df_cam_multi_morb, "hospital_37_HR") %>% dplyr::select("eid", "prognostic_index") %>% dplyr::rename(mm_score_PI = prognostic_index)
